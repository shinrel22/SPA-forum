<template>
  <div :disabled="disabled" class="form-control datepicker">
...
<script>
  ...
  const past = new Date('01-01-1000')
  const future = new Date('01-01-3000')
  // import $ from './utils/NodeList.js'

  export default {
    props: {
      test: {type: String, default: '1'},
      ...
      func: {type: Function},
      args: {type: Array, default () { return [] }},
      kwargs: {type: Object, default () { return {} }},
    ...
    watch: {
      ...
      val (val, old) {
        this.$emit('input', val)
        if (this.func) {
          this.func(this, val, this.name, ...this.args)
        }
      },
      value (val) {
    ...
    methods: {
      ...
      parse (str) {
        ...
        let date = str.length === 10 && (this.format === 'dd-MM-yyyy' || this.format === 'dd/MM/yyyy') ? new Date(str.substring(6, 10), str.substring(3, 5) - 1, str.substring(0, 2)) : new Date(str)
      ...
      limitDate (date, result) {
        if (this.limit) {
          if ((this.limit.from || past) > date || date > (this.limit.to || future)) {
            result = 'datepicker-item-disable'
          }
        }
        return result
      },
      getDateRange () {
        ...
        if (firstDayWeek > 1) {
          ...
          for (let i = 1; i < firstDayWeek; i++) {
            ...
            sclass = this.limitDate(date, sclass)
