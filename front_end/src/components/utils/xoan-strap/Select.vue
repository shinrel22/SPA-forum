<template>
  <div ref="select" :class="classes" v-click-outside="close">
    <div ref="btn" :class="size" class="form-control dropdown-toggle" tabindex="1" :disabled="disabled || !hasParent"
         :readonly="readonly"
    ...
    >
      <span class="btn-content"
            v-html="loading ? text.loading : showPlaceholder || (multiple && showCount ? selectedText : selected)"></span>
    ...
    <ul class="dropdown-menu"
        :style="dropDownReachMaxLength ? {'max-height': (32 * dropDownListMaxLength) + 'px', 'overflow': 'scroll'} : {}">
...
<script>
  ...
  export default {
    ...
    props: {
      size: {type: String, default: 'input-md'},
      func: {type: Function},
      dropDownListMaxLength: {type: Number, default: 10},
    ...
    computed: {
      ...
      dropDownReachMaxLength () {
        return this.filteredOptions.length > this.dropDownListMaxLength
      }
    },
    watch: {
      options (options) {
      ...
      val (val, old) {
        ...
        if (val !== old) {
          ...
          this.$emit('input', val)
          if (this.func) {
            this.func(this, val, this.name, true)
          }
        }
